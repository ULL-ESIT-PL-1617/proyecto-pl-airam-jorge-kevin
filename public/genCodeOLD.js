...
let assign_ = function(obj, result){
  for (let i = 0; i < result.assignations.length; i++){
    element_(obj, result.assignations[i].element);
    obj.code += " = ";
...
let element_ = function(obj, result) {
    if (typeof(result) == "string") {
        obj.code += "_" + result;
    } else if (result.type === "idAccess") {
        obj.code += result.base;
    } else { // type === "arrayAccess"
        obj.code += "_" + result.id;
        for (var i in result.index) {
            obj.code += "[" + result.index[i] + "]";
        }
    }
}
...
let declaration_ = function(obj, result){
  ...
  if(result.varType.array != undefined){
    for(let i = 0; i < result.assignations.length; i++){
      ...
      obj.code += "]" + ((i == (result.assignations.length - 1)) ? "; " : ", ");
    }
  }
  else{
    for(let i = 0; i < result.assignations.length; i++){
